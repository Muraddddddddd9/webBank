...
// Элемент для отопраженя курса валют
const elementUSD = document.querySelector('[data-value="USD"]')
const elementEUR = document.querySelector('[data-value="EUR"]')
const elementGBP = document.querySelector('[data-value="GBP"]')

// Элементы формы ввод, выбор валюты, поле с результатом
const input = document.querySelector('#input');
const result = document.querySelector('#result');
const selectCours = document.querySelector('#select');



getCurrencies()

// Функция получения валют и отображении страницы
async function getCurrencies() {
    const resppons = await fetch('https://www.cbr-xml-daily.ru/daily_json.js');
    const data = await resppons .json()
...
selectCours.oninput = convertValue
result.oninput = convertValueRub

// Функция конвертации 
function convertValue() {
    result.value = ((parseFloat(input.value) * rates[selectCours.value].Nominal) / rates[selectCours.value].Value).toFixed(2);
}

function convertValueRub() {
    input.value = ((parseFloat(result.value) / rates[selectCours.value].Nominal)* rates[selectCours.value].Value).toFixed(2);
}
